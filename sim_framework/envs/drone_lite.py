...
class DroneEnv(object):
    ...
    def __init__(self, random, headless=True, use_vision_sensor = False, seed = 42, state = "Normal", SCENE_FILE=None,
        reward_function_name = 'Normal', buffer_size=None, neta=0.9 ):

        if reward_function_name not in list(rew_functions.import_dict.keys()):
        ...
        ## Initial state (colocar em função)
        ...
        print('initial_orientation = ', self.initial_orientation)

        print("Initial Position of the Target = ", self.target.get_position())
        ...
        self.random=random
        ## timestep
        self.dt = np.round(sim.simGetSimulationTimeStep(),4)
        ...
        ## Creating table for weighted initializations
        ...
        ## Reward Functions
        self.weight_dict = rew_functions.import_dict[reward_function_name]['weight_dict']
        self.reward_function = rew_functions.import_dict[reward_function_name]['fun']
        self.reward_info = rew_functions.import_dict[reward_function_name]['info']
    ...
    def step(self, action):
      ...
      # Reward
        rel_pos, rel_or = self._get_reward_data()
        roll = rel_or[0]
        pitch = rel_or[1]
        yaw = rel_or[2]


        self.radius = math.sqrt(rel_pos[0]**2 + rel_pos[1]**2 + rel_pos[2]**2)
    ...
    def _reset_position(self, how = False):
        ...
        if how == "Gaussian":
            ...
            self._set_discretized_uniform_position()
        elif how == 'Weighted':
    ...
    def _set_discretized_uniform_position(self):
        ang_pos = np.zeros(3)
        for i in range(3):
            ang_pos[i] =  self.ang_ticks[np.random.choice(len(self.ang_ticks), 1)]
        self.agent.set_orientation(np.round(ang_pos,2).tolist())

        pos = np.zeros(3)
        for i in range(2):
            pos[i] = self.x_y_ticks[np.random.choice(len(self.x_y_ticks), 1)]

        pos[2] = self.z_ticks[np.random.choice(len(self.z_ticks), 1)] 
        self.agent.set_position(np.round(pos,2).tolist())

    def _set_gaussian_position(self):
        # position
        pos = np.zeros(3)
        for i in range(3):
            ...
            if i <= 1:
                ...
                pos[i] = z + 1.7
        self.agent.set_position(pos.tolist())
        # angular pos
        ang_pos = np.zeros(3)
        for i in range(3):
            z = random.gauss(0, 0.6)
            ang_pos[i] = z
        self.agent.set_orientation(ang_pos.tolist())



    def _set_uniform_position(self):
